...
struct echelon_packet {
	uint64_t tag;
	bytes data; };

size_t echelon_depacket(bytes in, struct echelon_packet * out, size_t length) {
	int i = 0;
	for(; i < length; i++) {
		uint64_t length;
		size_t n = readheader(in, &(out[i].tag), &length);
		out[i].data = bytescut(in, n, length);
		in = bytescut(in, n + length, 0); }
	return i; }

void echelon_decrypt(bytes in, bytes * out, 
	struct ec_group_st * curve, struct bignum_st * key) {


	uint64_t tag, length;
	int s = readheader(in, &tag, &length);
	in = bytescut(in, s, 0);

	in.raw[0] += 2;
	struct ec_point_st * secret = EC_POINT_new(curve);
	
	EC_POINT_oct2point(curve, secret, in.ptr, 33, 0);
	EC_POINT_mul(curve, secret, 0, secret, key, 0);
	...
	AES_ctr128_encrypt(in.ptr, out->ptr, in.length, &aeskey, iv, ecount, &num);
...
void decrypt(char ** argv) {
	...
	struct bignum_st * key = readkeyfile(argv[1]);
	struct ec_group_st * curve = EC_GROUP_new_by_curve_name(NID_secp256k1);

	bytes in = bytesalloc(1 << 15);
	int file = open(argv[0], O_RDONLY);	
	in.length = read(file, in.ptr, in.length);

	bytes out = bytesalloc(1 << 15);
	echelon_decrypt(in, &out, curve, key);
